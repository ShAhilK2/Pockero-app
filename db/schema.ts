...
export const savedItemsRelations = relations(savedItems, ({ one }) => ({
  // If a saved item was originally from RSS, link it
  rssArticle: one(rssArticles, {
    fields: [savedItems.url],
    references: [rssArticles.url],
  }),
}));

// RSS Articles relations
export const rssArticlesRelations = relations(rssArticles, ({ many }) => ({
  // Link to saved items if user saves an RSS article
  savedVersions: many(savedItems),
}));
